...
class OpenIntent:
    ...
    def start(self):
        """This intent controlls the model in that way, that the door is
        eventually open."""
    ...
    def _send_command_to_door(self):
        ...
        if self.door_model.state.__class__.__name__ == "OpenState":
            ...
            logger.debug("OpenIntent: Door is closed. Trigger opening.")
            ...
            logger.error("OpenIntent: unhandled door state %s", self.door_model.state.__class__.__name__)
            self.door_model.set_intent("Idle")
